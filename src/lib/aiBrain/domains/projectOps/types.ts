...
export interface ReadinessCrewConfirmation {
  ...
  questions_before_arrival: string;
...
// ─── Phase 3: Trade confirmation cutoff + material-dependency risk ─────────

export interface TradeConfirmationIssue {
...
export interface MaterialScheduleRisk {
  taskId: string;
  taskName: string;
  materialId: string;
  materialName: string;
  materialStatus: MaterialReadyStatus;
  expectedDeliveryDate: string | null;
  reason: string;
  /** Tasks that depend (directly or transitively) on the at-risk task — the whole point of a "schedule dependency" check, not just a same-day material gate (see readinessGates.ts's simpler checkMaterialsGate). */
  downstreamImpactedTaskIds: string[];
}

/** 100% deterministic — see tradeMaterialCoordination.ts. Whether something here is escalation-worthy is also a deterministic decision (shouldEscalate.ts logic), never left to AI. */
export interface TradeMaterialCoordinationResult {
  projectId: string;
  asOfDate: string;
...
export type EscalationChannel = 'ui' | 'voice' | 'email' | 'api';
